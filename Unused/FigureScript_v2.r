...
# 4. Richness Analysis: MODEL 2 and 5 + FIGURE 1 + SI complete richness plot
    ...
    # 4b. richness GLMM for metabolites - GLMM (model 5)
            ...
            alpha_dfmetab<-estimate_richness(btab_count, split = TRUE, measures = NULL) %>%
            merge(sample_data(btab_count), by=0)
            ...
            confs<-confint(Stats[["MetaboliteGLMMpH"]])
                # pH confints cross zero for all pH
...
# 5. Holobiont Distinctness: MODEL 3 AND 4 + FIGURE 2 +SI plot showing increasing compositional similarity
    ...
    # 5c1. Holobiome/Environmental distinct proportions sequence
        #Sequences distinct
                OverlapByArms<-list(
            ...
            #make plot    
                ...
                Figs[["ESVOverlap_poster"]] <- ggplot()+
                                            ...
                                            facet_wrap(~overlapType, labeller = labeller(overlapType = 
                                                            c("Holobiont Richness" = "Sequences")))
            #print poster figure 1
                #png("PosterHolobiontCommunityPlot.png", width = 7, height = 4, units = 'in', res = 2000)
                #Figs[["ESVOverlap_poster"]] # Make plot
                #dev.off()
    ...
    # 5f1. holbiont distinctness for specific holobiomes sequences 
        ...
        Figs[["SpecificHolobiontDistinctness_poster"]] <- ggplot(SpecificHolobiontDistinctness)+
                                            #geom_hline(yintercept=0.5, linetype='dotted')+
                                            #geom_violin(aes(x=pH, y=value, fill=Individual_Holobiont)) +
                                            geom_boxplot(aes(x=pH, y=value,fill=Individual_Holobiont))  +
                                            labs(   #title="FIGURE S6: Individual Holobiont Microbiome Distinctness - i.e. percentage of Holobiont microbiome richness not shared between the environmental microbiome and the individual holobiont microbiome",
                                                    #subtitle="CS=Halisarca sp.; RVS=Tethys sp.",
                                                    y="Individual Holobiont Distinctness", 
            ...
            #print poster figure 2
                png("SpecificHolobiontDistinctness_poster.png", width = 9, height = 6, units = 'in', res = 2000)
                Figs[["SpecificHolobiontDistinctness_poster"]] # Make plot
                dev.off()
...
# 6. Taxa barplots
    ...
    # metabolite indicator analysis
        #indicator species stats

                btab_holo<-prune_samples(sample_data(btab_p)$Sample=="SESSILE", btab_p)
                btab_holo<- prune_taxa( taxa_sums(btab_holo)>0, btab_holo)

                indval<-multipatt(
                                x=t(otu_table(btab_holo)), 
                                cluster=as.numeric(as.factor(paste0(sample_data(btab_holo)$pH#, 
                                                                    #sample_data(ps16p)$Site, 
                                                                    #sample_data(btab_p)$Sample 
                                                                    ))),
                                control=how(nperm=999))
                ...
                tax_table(btab_p) [ rownames(tax_table(btab_p)) %in% rownames(IndicatorSpecies100Fraction) ]


        #indicator species stats

                btab_holo<-prune_samples(sample_data(btab_p)$Sample=="Algae", btab_p)
                btab_holo<- prune_taxa( taxa_sums(btab_holo)>0, btab_holo)
                ...
                tax_table(btab_p)[,3][ rownames(tax_table(btab_p)) %in% rownames(IndicatorSpecies100Fraction) ]



        #indicator species stats

                btab_holo<-prune_samples(sample_data(btab_p)$Sample=="100-500UM", btab_p)
                btab_holo<- prune_taxa( taxa_sums(btab_holo)>0, btab_holo)
    ...
    #Mains figures
        ...
        #Figure2
           # grid.arrange(grobs = list(Figs[["ESVOverlap"]] ), ncol = 1) ## display plot
            grid::grid.newpage()
            grid::grid.draw(cbind(ggplotGrob(Figs[["ESVOverlap_poster"]]),  ggplotGrob(Figs[["MetaboliteOverlap"]])))
